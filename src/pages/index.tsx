...
const Sketch = (): JSX.Element => {
  ...
  const verticalColorScale = useMemo(() => chroma.scale(verticalPalette.colors), [verticalPalette])
  const horizontalColorScale = useMemo(() => chroma.scale(horizontalPalette.colors), [horizontalPalette])

  let seed = 666
  const projectNoise = (noise2d: Noise2D) => (noise3d: Noise3D): Noise2D => {
  ...
  const Grid = ({
    ...
    const outlines: RefObject<InstancedMesh> = useRef<InstancedMesh>(null)
    const { size } = useThree()
    const { cols, rows, count, top, left, bottom, right } = useMemo(() => {
    ...
    const coords = useCallback((i: number) => {
      const col = i % cols
    ...
    const noiseArray = useCallback((noise: Noise2D) => new Float32Array(Array.from({ length: count }, (_, i) => {
      const { x, y } = coords(i)
      return noise(x, y)
    })), [coords, count])
    ...
    const colors = useMemo(() => Array.from({ length: count }, (_, i) => {
      const col = i % cols
      const row = Math.floor(i / cols)
      return c.set(chroma.mix(horizontalColors(col / cols), verticalColors(row / rows)).hex()).clone()
    }), [count, cols, rows, horizontalColors, verticalColors])

    const noiseArrays = useMemo(() => Object.fromEntries(Object.entries(noises).map(([name, noise]) => [name, noiseArray(noise).map(n => (n + 1) / 2)])), [noises])
    ...
    useLayoutEffect(() => {
      for (let col = 0; col < cols; col++) {
        for (let row = 0; row < rows; row++) {
          const x = left + col * scale
          const y = top + row * scale
          o.position.set(x + (scale / 2), y + (scale / 2), depthNoise(x, y) * scale / 2)
          o.updateMatrix()
      ...
      <group>
        <instancedMesh ref={cells} args={[undefined, undefined, count]}>
          ...
          {Object.entries(noiseArrays).map(([name, array]) => (
            <instancedBufferAttribute key={name} attach={`material-${name}`} args={[array, 1]} />
          ))}
        </instancedMesh>
    ...
    <Layout meta={{ title: 'Hello, World' }}>
      <Box sx={{ position: 'relative', width: '100%', height: '100%' }}>
        <Canvas frameloop='demand' camera={{ position: [0, 0, 750] }}>
          ...
          <pointLight position={[-2000, 3000, 2000]} decay={0.005} intensity={Math.PI} castShadow />
          <CameraControls />
          <Grid scale={gridScale} />
        </Canvas>
        <ConfigMenu onSubmit={(event: React.FormEvent) => event.preventDefault()}>
          <ConfigField as={Select} label='Horizontal Palette' name='horizontalPalette' value={horizontalPalette.name} onChange={({ target: { value } }) => setHorizontalPalette(get(value as PaletteName))}>
            {getNames().map((name, i) => (<option key={`h-${name}-${i}`} value={name}>{name}</option>))}
          </ConfigField>
          <ConfigField as={Select} label='Vertical Palette' name='VerticalPalette' value={verticalPalette.name} onChange={({ target: { value } }) => setVerticalPalette(get(value as PaletteName))}>
            {getNames().map((name, i) => (<option key={`v-${name}-${i}`} value={name}>{name}</option>))}
          </ConfigField>
          <ConfigField as={Checkbox} label='Show Grid' name='gridShown' checked={gridShown} onChange={() => showGrid(!gridShown)} />
          <ConfigField as={Slider} label='Grid Scale' name='gridScale' min={75} max={200} step={10} value={gridScale} onChange={({ target: { value } }) => setGridScale(Number.parseInt(value))} />
        </ConfigMenu>
      </Box>
    </Layout>
  )
