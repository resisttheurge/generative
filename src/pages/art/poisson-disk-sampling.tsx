...
const PDS: React.FC = () => {
  ...
  const config = useCallback(
    (width: number, height: number) =>
      Generator.record({
        ...
        pds: blueNoise({
          ...
          padding: radius + offset,
          candidateLimit,
        }),
      }).map(({ noises: { frameNoise, shadowNoise, colorNoise }, pds }) => {
    ...
    [palette, offset, radius, multiplier, candidateLimit, noiseZoom],
  )
  ...
  const setup: PaperSetup = useCallback(
    ...
    [generate, config, palette.background],
  )
      ...
      <Box sx={{ position: 'relative', width: '100%', height: '100%' }}>
        ...
        >
          ...
          <ConfigField
            ...
            onChange={({ target: { value } }) =>
              setMultiplier(Number.parseFloat(value))
            }
          />
          <ConfigField
            label={`Buffer: ${buffer}`}
            ...
            defaultValue={buffer}
            onChange={({ target: { value } }) =>
              setBuffer(Number.parseFloat(value))
            }
          />
          <ConfigField
            label={`Max Samples: ${candidateLimit}`}
            as={Slider}
            name='candidateLimit'
            min={15}
            max={45}
            ...
            onChange={({ target: { value } }) =>
              setCandidateLimit(Number.parseInt(value))
            }
          />
          <ConfigField
