...
describe('the hashStates function', () => {
  it.prop({
    ...
    ({ hasher, state, count }) => {
      expect(() => hashStates(hasher, state, count)).toThrow(
        NEGATIVE_COUNT_ERROR(count),
      )
    },
  )
  ...
  it.prop({
    ...
    ({ hasher, state, count }) => {
      expect(hashStates(hasher, state, count)).toHaveLength(Math.ceil(count))
    },
  )
  ...
  it.prop({
    ...
    ({ hasher, state, count }) => {
      expect(hashStates(hasher, state, count)[0]).toBe(state)
    },
  )
  ...
  it.prop({
    hasher: arb.hasher(),
    state: fc.anything(),
    nums: fc.integer({ min: 2, max: GENERATION_LIMIT }).chain(count =>
      fc.record({
...
describe('the initStates function', () => {
  it.prop({
    ...
    'should throw an error if count is negative or NaN',
    ({ hasher, seed, count }) => {
      expect(() => initStates(hasher, seed, count)).toThrow(
        NEGATIVE_COUNT_ERROR(count),
      )
    },
  )
  ...
  it.prop({
    ...
    ({ hasher, seed, count }) => {
      expect(initStates(hasher, seed, count)).toEqual(
        hashStates(hasher, hasher.initState(seed), count),
      )
    },
  )
})
