...
describe('an arbitrary hasher', () => {
  it.prop([arb.hasher()])('should match the Hasher interface', hasher => {
    expect(hasher.name === undefined || typeof hasher.name === 'string').toBe(
      true,
    )
  ...
  it.prop({ name: fc.string() })(
    ...
    ({ name }) => {
      ...
      const arbitrary = fc.sample(
        arb.hasher(undefined, { name: fc.constant(name) }),
        1,
      )[0]
