...
import {
  ...
  fill,
  fromJS,
} from '../data-structures/nd-arrays'
...
export type BlueNoiseGenerator<Dimensions extends number> = Generator<
...
export interface BlueNoiseState<Dimensions extends number>
  extends BlueNoiseStateProperties<Dimensions> {
  readonly generateActiveSample: Generator<{
    activeIndex: number
...
export class BlueNoiseLibrary<Dimensions extends number> {
  readonly State: new (
    initialValue: Vector<Dimensions> | BlueNoiseStateProperties<Dimensions>,
  ) => BlueNoiseState<Dimensions> = (() => {
    ...
    return class implements BlueNoiseState<Dimensions> {
      ...
      constructor(
        ...
        if (Array.isArray(initialValue)) {
          invariant(
            library.inBounds(initialValue),
            () =>
              `initial position ([${printVector(initialValue)}]) must be in bounds`,
          )
          this.samples = List([initialValue])
          this.active = List([0])
          this.grid = setIn(
            library.emptyGrid,
            library.gridCoordinates(initialValue),
            0,
          )
        } else {
        ...
        this.generateActiveSample = Generator.nat(
          this.active.size > 0 ? this.active.size - 1 : 0,
        ).map(activeIndex => {
          const sampleIdx = this.active.get(activeIndex)
          invariant(
            typeof sampleIdx === 'number',
            () =>
              `index (${activeIndex}) not in active list ([${this.active.toJS().join(', ')}])`,
          )
          const sample = this.samples.get(sampleIdx)
          invariant(
            Array.isArray(sample),
            () =>
              `index (${sampleIdx}) not in sample list ([${this.samples.toJS().join(', ')}])`,
          )
        ...
        return library
          ...
          .map(idx => {
            ...
            invariant(
              neighbor !== undefined,
              () =>
                `neighbor ${idx} not found in sample list ([${samples.toJS().join(', ')}])`,
            )
            return neighbor
          })
      }
      ...
      canEmit(sample: Vector<Dimensions>): boolean {
        ...
        const canEmit =
          ...
          this.neighbors(sample).filter(
            neighbor => distanceSquared(sample, neighbor) < radiusSquared,
          ).length === 0
        return canEmit
      }
  ...
  constructor({
    ...
    padding = 0,
    candidateLimit = 30,
    initialPosition = Generator.tuple(
    ...
    this.gridDimensions = map(this.dimensions, dimension =>
      Math.ceil(dimension / this.cellSize),
    )
    this.emptyGrid = fromJS(fill(-1 as number, this.gridDimensions))
    this.generateCandidateAroundOrigin = nShell(this.n, {
      min: this.radius,
      max: 2 * this.radius,
    })
  ...
  inBounds(position: Vector<Dimensions>): boolean {
    return position.every(
      (n, i) => n >= 0 + this.padding && n <= this.dimensions[i] - this.padding,
    )
  }

  generateCandidateAround(
    position: Vector<Dimensions>,
  ): Generator<Vector<Dimensions>> {
    return this.generateCandidateAroundOrigin.map(candidate =>
      translate(candidate, position),
    )
  }
  ...
  neighborAddresses(position: Vector<Dimensions>): Array<Vector<Dimensions>> {
    invariant(
      ...
      () => `position ([${position.join(', ')}]) not in bounds`,
    )
  ...
  generateBlueNoise(
    initialPosition: Vector<Dimensions>,
  ): BlueNoiseGenerator<Dimensions> {
    invariant(
      this.inBounds(initialPosition),
      () => `initial position ([${initialPosition.join(', ')}]) not in bounds`,
    )
    return new Generator(rngState => {
      let currentRngState = rngState
      let currentNoiseState = new this.State(initialPosition)
      while (currentNoiseState.active.size > 0) {
        const [idxRngState, { activeIndex, sample }] =
          currentNoiseState.generateActiveSample.run(currentRngState)
        currentRngState = idxRngState
        let count
        for (count = 0; count < this.candidateLimit; count++) {
          const [nextCandidateRngState, nextCandidate] =
            this.generateCandidateAround(sample).run(currentRngState)
          currentRngState = nextCandidateRngState
          if (
            this.inBounds(nextCandidate) &&
            currentNoiseState.canEmit(nextCandidate)
          ) {
...
export function blueNoise<Dimensions extends number>(
  config: BlueNoiseConfig<Dimensions>,
  initialPosition?: Vector<Dimensions>,
  ...
  if (initialPosition !== undefined) {
    ...
    return library.generateInitialPosition.flatMap(
      library.generateBlueNoise.bind(library),
    )
  }
}
