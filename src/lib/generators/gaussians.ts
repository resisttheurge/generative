 ...
 */
export const marsagliaPolarMethod: Generator<Vector<2>> =
  Generator.tuple([
...
export function gaussianPair ({
  ...
  generator = (mu !== 0 && sigma !== 1) // bake in a linear transformation if requested
  ...
  return truncate !== undefined
    ? extras === 'discard'
      ...
      : Generator.spread(generator)
        .filter(inRange(truncate))
        .repeat(2)
    : generator
}

export function gaussian (config: Partial<NormalConfig> = {}): Generator<number> {
