...
export const validRadiusRange: () => fc.Arbitrary<Partial<Range>> = () =>
  fc.tuple(validRadiusBound(), validRadiusBound()).map(([a, b]) => {
...
export const invalidRadiusRange: () => fc.Arbitrary<Partial<Range>> = () =>
  fc.oneof(
    ...
    fc.record({
      min: invalidRadiusRangeBound(),
      max: invalidRadiusRangeBound(),
    }),
    // min is greater than max
    validRadiusBound().chain((max?: number) =>
      fc.record({
        min:
          max !== undefined
            ? fc
                ...
                .filter(n => n > max)
            : fc
                .double({ min: 1, noNaN: true, noDefaultInfinity: true })
                .filter(n => n > 1),
        max: fc.constant(max),
      }),
    ),
    // max is less than min
    validRadiusBound().chain((min?: number) =>
      fc.record({
        min: fc.constant(min),
        max:
          min !== undefined
            ? fc
                .double({ max: min, noNaN: true, noDefaultInfinity: true })
                .filter(n => n < min)
            : fc
                .double({ max: 0, noNaN: true, noDefaultInfinity: true })
                .filter(n => n < 0),
      }),
    ),
  )
