...
describe('the calculateState function', () => {
  ...
  it.prop({
    ...
    ({ prng, seed, offset, path, iteration }) => {
      invariant(prng.hashState !== undefined, () =>
        VARIATION_NOT_SUPPORTED(prng.name),
      )
      expect(calculateState(prng, seed, offset, path.push(iteration))).toBe(
        prng.hashState(calculateState(prng, seed, offset, path, iteration)),
      )
    },
  )
  ...
  it.prop({
    ...
    ({ prng, seed, offset, path, iteration }) => {
      expect(calculateState(prng, seed, offset, path, iteration + 1)).toBe(
        prng.nextState(calculateState(prng, seed, offset, path, iteration)),
      )
    },
  )
})
...
describe('the ConcretePRN class', () => {
  it.prop({
    ...
    seed: fc.string(),
    offset: fc.nat(100),
    path: fc.option(fc.array(fc.nat(100), { size: 'small' }).map(List), {
      nil: undefined,
    }),
  ...
  it.prop([arb.prn()])(
    ...
    prn => {
      expect(prn.normalized).toBe(
        (prn.value - prn.prng.min) / (prn.prng.max - prn.prng.min),
      )
    },
  )
  ...
  it.prop([arb.prn()])(
    ...
    prn => {
      expect(prn.toString()).toBe(
        `<${prn.prng.name ?? ANONYMOUS_PRNG_NAME}@${prn.seed}^${
          prn.offset
        }:${prn.path.push(prn.iteration).join(':')}=${
          prn.prng.formatState?.(prn.state) ?? JSON.stringify(prn.state)
        }->${prn.value}|${prn.normalized}>`,
      )
    },
  )
})
