...
import { jsf32, jsf32State } from './arbitraries'

describe('JSF32', () => {
  it.prop([jsf32()])('should implement the PRNG interface', prng => {
    expect(prng).toHaveProperty('name')
    expect(prng.name).toMatch(/jsf32[ab]/)
  ...
  it.prop({ prng: jsf32(), seed: fc.string() })(
  ...
  it.prop({ prng: jsf32(), seed1: fc.string(), seed2: fc.string() })(
  ...
  it.prop({ prng: jsf32(), state: jsf32State() })(
  ...
  it.prop({ prng: jsf32(), state1: jsf32State(), state2: jsf32State() })(
