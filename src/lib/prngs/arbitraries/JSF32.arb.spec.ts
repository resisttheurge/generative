...
describe('an arbitrary JSF32 implementation', () => {
  ...
  it.prop({
    ...
    ({ hasher, seed, state }) => {
      ...
      const arbitrary = fc.sample(arb.jsf32({ hasher: fc.constant(hasher) }), 1)[0]
  ...
  it.prop({ algorithm: fc.constantFrom(...impl.jsf32Algorithms) })(
    ...
    ({ algorithm }) => {
      ...
      const arbitrary = fc.sample(arb.jsf32({ algorithm: fc.constant(algorithm) }), 1)[0]
