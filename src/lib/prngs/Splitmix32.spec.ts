...
import { splitmix32 } from './arbitraries'

describe('Splitmix32', () => {
  it.prop([splitmix32()])('should implement the PRNG interface', prng => {
    expect(prng).toHaveProperty('name')
    expect(prng.name).toMatch(/splitmix32[ab]/)
  ...
  it.prop({ prng: splitmix32(), seed: fc.string() })(
  ...
  it.prop({ prng: splitmix32(), seed1: fc.string(), seed2: fc.string() })(
  ...
  it.prop({ prng: splitmix32(), state: fc.integer() })(
  ...
  it.prop({ prng: splitmix32(), state1: fc.integer(), state2: fc.integer() })(
