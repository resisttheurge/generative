...
export const contains = curry((bounds: Bounds, query: Query): boolean =>
  Array.isArray(query)
    ? query[0] >= bounds.min[0] &&
      query[1] >= bounds.min[1] &&
      query[0] <= bounds.max[0] &&
      query[1] <= bounds.max[1]
    : contains(bounds, query.min) && contains(bounds, query.max),
)

// From https://stackoverflow.com/a/306332
// demo: https://silentmatt.com/rectangle-intersection/
export const intersects = curry((bounds: Bounds, query: Query): boolean =>
  Array.isArray(query)
