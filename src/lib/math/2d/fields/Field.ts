...
export class Field<Data = undefined> {
  ...
  constructor (readonly cells: Array<Cell<Data>>, indexData?: ArrayBuffer) {
    this.cells = cells
    ...
    if (indexData !== undefined) {
      ...
      this.cells.forEach(cell => this.index.add(...cell.bounds.min, ...cell.bounds.max))
      this.index.finish()
    }
  ...
  contains (query: Query): boolean {
    return contains(this.bounds, query) &&
      this.find(
        query,
        cell => cell.contains(query)
      ) !== undefined
  }

  find (query: Query, filter?: (cell: Cell<Data>) => boolean): Cell<Data> | undefined {
    return this.findAll(
      query,
      filter
    )[0]
  ...
  findAll (query: Query, filter?: (cell: Cell<Data>) => boolean): Array<Cell<Data>> {
    const idxFilter = filter !== undefined
    ...
    return this.index.search(...min, ...max, idxFilter)
  ...
  neighbors (query: Query, { maxResults, maxDistance, filter }: NeighborConfig<Data> = {}): Array<Cell<Data>> {
    const position = !Array.isArray(query)
      ? centerPoint(query)
      : query
    ...
    return this.index.neighbors(...position, maxResults, maxDistance, idxFilter)
      .map(idx => this.cells[idx])
  }

  generate <T> (fn: (cell: Cell<Data>) => Generator<T>): Generator<Field<T>> {
    return Generator.tuple(this.cells.map(cell => cell.generate(fn)))
      .map(cells => new Field(cells, this.index.data))
  }
}
