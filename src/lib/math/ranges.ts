...
export const midpoint = (range: Range): number => {
  ...
  return isEmpty(range) ? range.min : (range.min + range.max) / 2
}
...
export const split = (range: Range, n?: number): [Range, Range] => {
  ...
  invariant(
    inRange(range, splitPoint),
    `split point n (${splitPoint}) must be in range (${toString(range)})`,
  )
  return isEmpty(range)
    ? [range, range]
    : [
        { min: range.min, max: splitPoint },
        { min: splitPoint, max: range.max },
      ]
}
  ...
  if (count === 1) return [range]
    ...
    for (let i = 0; i < count; i++) {
      result.push({
        ...
        max: i + 1 < count ? (i + 1) * step + range.min : range.max,
      })
    }
    return result
  }
