...
export function map <N extends number> (v: Vector<N>, fn: (n: number, i: number) => number): Vector<N> {
  return stMap(v, fn)
}
...
export function translate <N extends number> (v: Vector<N>, off: number | Vector<N>): Vector<N> {
...
export function transform <N extends number> (v: Vector<N>, s: number, off: number | Vector<N>): Vector<N> {
...
export function normalize <N extends number> (v: Vector<N>, factor: number = 1): Vector<N> {
  return scale(v, factor / magnitude(v))
}

export function distanceSquared <N extends number> (a: Vector<N>, b: Vector<N>): number {
  return a.reduce(
    (acc, next, i) => {
      const diff = next - b[i]
